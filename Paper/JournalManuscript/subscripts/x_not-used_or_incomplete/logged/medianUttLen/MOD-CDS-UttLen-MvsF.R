...
fcd.dur.bas <-  lmer(mutt_len.lg ~ (1|Corpus), data = f.cdsln)


# 1. Single-predictor effects ####
# Nothing to add
# Individual models ####
fcd.dur.age <-  lmer(mutt_len.lg ~ agem.c +
                      (1|Corpus), data = f.cdsln)
anova(fcd.dur.bas, fcd.dur.age)
                # no improvement

fcd.dur.cgd <-  lmer(mutt_len.lg ~ chi_gender +
                       (1|Corpus), data = f.cdsln)
anova(fcd.dur.bas, fcd.dur.cgd)
                # no improvement

fcd.dur.med <-  lmer(mutt_len.lg ~ mat_ed_num3 +
                       (1|Corpus), data = f.cdsln)
anova(fcd.dur.bas, fcd.dur.med)
                # no improvement

fcd.dur.myr <-  lmer(mutt_len.lg ~ mother_dob +
                       (1|Corpus), data = f.cdsln)
anova(fcd.dur.bas, fcd.dur.myr)
                # no improvement; anova throws convergence warning

fcd.dur.nsb <-  lmer(mutt_len.lg ~ number_older_sibs +
                       (1|Corpus), data = f.cdsln)
...
fcd.dur.mednsb <-  lmer(mutt_len.lg ~
...
# Best model: ####
fcd.dur.best <- lmer(mutt_len.lg ~ (1|Corpus),
                 data = f.cdsln)


# MALES ####
# 0. Random effects only (base model) ####
mcd.dur.bas <-  lmer(mutt_len.lg ~ (1|Corpus), data = m.cdsln)


# 1. Single-predictor effects ####
# Nothing to add
# Individual models ####
mcd.dur.age <-  lmer(mutt_len.lg ~ agem.c +
                      (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.age)
                # no improvement

mcd.dur.cgd <-  lmer(mutt_len.lg ~ chi_gender +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.cgd)
                # no improvement

mcd.dur.med <-  lmer(mutt_len.lg ~ mat_ed_num3 +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.med)
                # no improvement

mcd.dur.myr <-  lmer(mutt_len.lg ~ mother_dob +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.myr)
                # no improvement

mcd.dur.nsb <-  lmer(mutt_len.lg ~ number_older_sibs +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.nsb)
                # only marginal improvement (p = 0.07343)


# 2. Try out 2-way interactions ####
# Nothing to add
# Individual models ####
mcd.dur.agecgd <-  lmer(mutt_len.lg ~
                           agem.c:chi_gender +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agecgd)
                # no improvement

mcd.dur.agemed <-  lmer(mutt_len.lg ~
                           agem.c:mat_ed_num3 +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agemed)
                # no improvement

mcd.dur.agemyr <-  lmer(mutt_len.lg ~
                           agem.c:mother_dob +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agemyr)
                # no improvement

mcd.dur.agensb <-  lmer(mutt_len.lg ~
                           agem.c:number_older_sibs +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agensb)
                # no improvement

mcd.dur.cgdmed <-  lmer(mutt_len.lg ~
                           chi_gender:mat_ed_num3 +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.cgdmed)
                # no improvement

mcd.dur.cgdmyr <-  lmer(mutt_len.lg ~
                           chi_gender:mother_dob +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.cgdmyr)
                # no improvement

mcd.dur.cgdnsb <-  lmer(mutt_len.lg ~
                           chi_gender:number_older_sibs +
                       (1|Corpus), data = m.cdsln)
...
mcd.dur.mednsb <-  lmer(mutt_len.lg ~
                           mat_ed_num3:number_older_sibs +
                       (1|Corpus), data = m.cdsln)
...
# Nothing to add
# Individual models ####
mcd.dur.agecgdmed <-  lmer(mutt_len.lg ~
                           agem.c:chi_gender:mat_ed_num3 +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agecgdmed)
                # no improvement

mcd.dur.agecgdmyr <-  lmer(mutt_len.lg ~
                           agem.c:chi_gender:mother_dob +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agecgdmyr)
                # no improvement

mcd.dur.agecgdnsb <-  lmer(mutt_len.lg ~
                           agem.c:chi_gender:number_older_sibs +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agecgdnsb)
                # no improvement

mcd.dur.agemednsb <-  lmer(mutt_len.lg ~
                           agem.c:mat_ed_num3:number_older_sibs +
                       (1|Corpus), data = m.cdsln)
anova(mcd.dur.bas, mcd.dur.agemednsb)
                # no improvement

mcd.dur.cgdmednsb <-  lmer(mutt_len.lg ~
                           chi_gender:mat_ed_num3:number_older_sibs +
                       (1|Corpus), data = m.cdsln)
...
# Best model: ####
mcd.dur.best <- lmer(mutt_len.lg ~ (1|Corpus),
                 data = m.cdsln)
