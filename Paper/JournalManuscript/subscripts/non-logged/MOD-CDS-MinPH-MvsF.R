...
fcd.bas <-  lmer(cds.minph ~ 1 + (1|Corpus), data = f.cdsrd, REML = F)
...
fcd.mph.nsb <-  lmer(cds.minph ~ n_sibs +
                       (1|Corpus), data = f.cdsrd, REML = F)
...
# Best model: ####
fcd.mph.best <- lmer(cds.minph ~ 1 + (1|Corpus), data = f.cdsrd, REML = T)


# MALES ####
# 0. Random effects only (base model) ####
mcd.bas <-  lmer(cds.minph ~ 1 + (1|Corpus), data = m.cdsrd, REML = F)
...
mcd.mph.nsb <-  lmer(cds.minph ~ n_sibs +
                       (1|Corpus), data = m.cdsrd, REML = F)
...
# Best model: ####
mcd.mph.best <- lmer(cds.minph ~ 1 + (1|Corpus), data = m.cdsrd, REML = T)
