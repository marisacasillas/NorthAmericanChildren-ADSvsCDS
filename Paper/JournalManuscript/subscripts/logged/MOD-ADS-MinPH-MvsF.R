...
fad.bas <-  lmer(ads.minph.lg ~ 1 + (1|Corpus), data = f.adsrd, REML = F)
...
# MALES ####
# 0. Random effects only (base model) ####
mad.bas <-  lmer(ads.minph.lg ~ 1 + (1|Corpus), data = m.adsrd, REML = F)
...
# Best model: ####
mad.mph.best <- lmer(ads.minph.lg ~ 1 + (1|Corpus), data = m.adsrd, REML = T)
